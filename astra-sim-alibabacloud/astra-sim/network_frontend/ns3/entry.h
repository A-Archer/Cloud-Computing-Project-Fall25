...
// ==========================================
// 4. MARC Controller Class Definition
// ==========================================
class MarcController {
public:
    struct AlgoNode {
        int id;
        int ringLevel; // Hops from source (Ring Layer)
        std::vector<int> neighbors;
    };

    std::map<int, AlgoNode> graph;

    // 1. Build topology graph from NS-3 physical topology
    void BuildGraph() {
        ...
        for (uint32_t i = 0; i < NodeList::GetNNodes(); ++i) {
            ...
            for (uint32_t j = 0; j < node->GetNDevices(); ++j) {
                ...
                // Traverse other nodes on the channel
    ...
    void ComputeTree(int sourceId, std::vector<int> destinations) {
        ...
        for (int r = maxRing; r > 0; r--) {
            ...
            // Identify candidate parents in previous ring
            ...
            while (covered.size() < targets.size()) {
                ...
                if (bestCand != -1) {
                    activeNodes.insert(bestCand); // Activate parent
                    totalSwitchesUsed++;
...
void notify_sender_packet_arrivered_receiver(int sender_node, int receiver_node, uint64_t message_size, AstraSim::ncclFlowTag flowTag) {
...
void qp_finish(FILE *fout, Ptr<RdmaQueuePair> q) {
  ...
  uint64_t notify_size;
  {
    ...
    if (sender_src_port_map.find(make_pair(q->sport, make_pair(sid, did))) == sender_src_port_map.end()) {
...
// ==========================================
// MAIN FUNCTION (with MARC Integration)
// ==========================================
int main1(string network_topo,string network_conf) {
  ...
  // ================= MARC ALGORITHM INTEGRATION =================
  {
      ...
      // 2. Build graph from NS-3 topology
      marc.BuildGraph();

      // 3. Define destinations (e.g., broadcast to the first 16 nodes)
      // Note: we dynamically scan the network to determine valid targets and avoid out-of-range indices
      std::vector<int> destinations;
      uint32_t n_nodes = NodeList::GetNNodes();
      
      // Simple heuristic: assume the first 16 nodes are GPUs (or end hosts)
      // For demonstration, we choose nodes with small IDs
      for (uint32_t i = 1; i < std::min(n_nodes, (uint32_t)16); ++i) {
          destinations.push_back(i);
      }

      // 4. Execute Ring-Peeling algorithm
